    ...
    <div>
        ...
        >
            <ImportExportBegBal
                ...
                @closeSuccess="closeSuccessImportExportModal"
            />
        </Transition>
        <Transition
            enter-active-class="transition ease-out duration-300"
        ...
        >
            ...
            >
                <thead class="sticky top-0 z-10">
                    <tr>
                        <th
                            class="px-3 py-3 w-[20%] text-left font-semibold tracking-wider uppercase"
                        >
                        ...
                        <th
                            class="px-3 py-3 w-[35%] text-left font-semibold tracking-wider uppercase"
                        >
                            Customer Name
                        </th>
                        <th
                            class="px-3 py-3 w-[15%] text-center font-semibold tracking-wider uppercase"
                        >
                            Balance
                        </th>
                        <th
                            class="px-3 py-3 w-[10%] text-center font-semibold tracking-wider uppercase"
                        >
                            Action
                        </th>
                    </tr>
                ...
                <tbody v-else>
                    <tr
                        v-for="beginningbalance in beginningbalances.data"
                        :key="beginningbalance.id"
                        class="hover:bg-[var(--color-primary)]/20 transition-colors duration-150 group h-10"
                    >
                        <td class="px-3 py-1 font-medium">
                            {{ beginningbalance.beginningbalance_no }}
                        </td>
                        <td class="px-3 py-1">
                            {{ formatDate(beginningbalance.receipt_date) }}
                        </td>
                        <td class="px-3 py-1 font-medium">
                            <div class="flex items-center gap-3">
                                ...
                                >
                                    {{
                                        getFirstValidChar(beginningbalance.name)
                                    }}
                                </div>
                                <div>
                                    <div class="font-medium">
                                        {{ beginningbalance.name }}
                                    </div>
                                    <div
                                        class="text-xs text-[var(--color-text-secondary)] mt-0.5"
                                    >
                                        {{
                                            beginningbalance.customer_code ||
                                            "No TIN"
                                        }}
                                    </div>
                                </div>
                        ...
                        <td class="px-3 py-1 text-right">
                            {{
                                formatCurrency(beginningbalance.balance_amount)
                            }}
                        </td>
                        <td class="px-3 py-1">
                            <div class="flex justify-center gap-2">
                                <button
                                    @click="openViewModal(beginningbalance)"
                    ...
                    <!-- Empty State -->
                    <tr
                        v-if="!isLoading && beginningbalances.data.length === 0"
                    >
            ...
            <!-- Pagination Links -->
            <div v-if="isLoading || beginningbalances.data.length === 0" />
            <div v-else>
                <PaginationLinks :paginator="beginningbalances" />
            </div>
        </div>
    </div>
</template>
...
import AddBeginningBalance from "../Modals/TransactionModals/AddBeginningBalance.vue";
import ViewBeginningBalance from "../Modals/TransactionModals/ViewBeginningBalance.vue";
import ImportExportBegBal from "../Modals/TransactionModals/ImportExportBegBal.vue";
import {
    mdiClose,
    mdiEye,
...
const closeSuccessModal = () => {
    showModal.value = false; // Close the modal
    showSuccessToast("Beginning Balance Has Been Added Successfully");
};
...
const closeImportExportModal = () => {
    showImportExportModal.value = false; // Close the modal
};
const closeSuccessImportExportModal = () => {
    showImportExportModal.value = false; // Close the modal
    showSuccessToast("Beginning Balance Has Been Added Successfully");
};
