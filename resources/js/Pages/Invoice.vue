...
const handleConfirm = async (confirmed) => {
    ...
    if (confirmed && pendingDeleteID.value) {
        try {
            router.delete(route("deleteInvoice", pendingDeleteID.value), {
                onSuccess: () => {
...
const performSearch = debounce((q) => {
    ...
    router.get(route("invoice"), filters, {
        preserveState: true,
        replace: true,
        onFinish: () => {
